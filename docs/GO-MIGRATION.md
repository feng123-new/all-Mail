...
The Go process owns the public listener, SPA delivery, request IDs, security headers, liveness/readiness endpoints, metrics, the explicit Go migration command, and the new runtime table contracts. Existing Fastify modules remain the authoritative writers for current application tables.
...
`go-jobs` is intentionally behind the `go-migration` Compose profile because it is still a heartbeat-only supervisor. Start it only while developing the first real Go worker:

```bash
docker compose --profile go-migration up -d go-jobs
```
...
10. records the checksum only after successful execution.
...
1. Port log retention to Go and verify deletion boundaries.
2. Port forwarding claim/send/update with provider idempotency and lease recovery.
3. Implement outbound delivery jobs and attempt history.
4. Implement Gmail History and Microsoft Graph delta synchronization.
5. Implement IMAP UID/UIDVALIDITY synchronization.
6. Port API-key external allocation/read endpoints.
7. Port ingress replay protection and signed delivery.
8. Port admin and mailbox portal authentication last.
...
1. stop `app` and any `go-jobs` profile service;
