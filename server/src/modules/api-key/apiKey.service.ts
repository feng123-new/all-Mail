...
export const apiKeyService = {
    ...
    async create(input: CreateApiKeyInput, createdBy: number) {
        ...
        const allowedDomainIds = normalizeDistinctPositiveIds(input.allowedDomainIds);
        ...
        if (allowedDomainIds && allowedDomainIds.length > 0) {
            const count = await prisma.domain.count({ where: { id: { in: allowedDomainIds } } });
            if (count !== allowedDomainIds.length) {
                throw new AppError('DOMAIN_NOT_FOUND', 'Some allowed domains do not exist', 404);
            }
        }
        ...
        const apiKey = await prisma.apiKey.create({
            data: {
                ...
                allowedDomainIds: toNullableJsonIds(allowedDomainIds),
                createdBy,
            },
            select: {
                id: true,
        ...
        return {
            ...
            allowedDomainIds: parseJsonIdList(apiKey.allowedDomainIds),
            key,
        };
    },
    ...
    async update(id: number, input: UpdateApiKeyInput) {
        ...
        if (input.allowedDomainIds !== undefined) {
            const allowedDomainIds = normalizeDistinctPositiveIds(input.allowedDomainIds) || [];
            ...
            updateData.allowedDomainIds = toNullableJsonIds(allowedDomainIds);
        }
