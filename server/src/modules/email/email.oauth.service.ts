...
import { getDefaultAuthType, mergeProviderConfig } from '../mail/providers/types.js';

const GOOGLE_AUTH_URL = 'https://accounts.google.com/o/oauth2/v2/auth';
const GOOGLE_TOKEN_URL = 'https://oauth2.googleapis.com/token';
const GOOGLE_USERINFO_URL = 'https://openidconnect.googleapis.com/v1/userinfo';
const GMAIL_PROFILE_URL = 'https://gmail.googleapis.com/gmail/v1/users/me/profile';
const MICROSOFT_GRAPH_ME_URL = 'https://graph.microsoft.com/v1.0/me?$select=mail,userPrincipalName';
...
const localOAuthStateStore = new Map<string, OAuthStateRecord & { expiresAt: number }>();
...
async function saveOAuthState(state: string, value: OAuthStateRecord): Promise<void> {
  const redis = getRedis();
  const payload = JSON.stringify(value);
  if (redis) {
    try {
      await redis.setex(buildOAuthStateKey(state), OAUTH_STATE_TTL_SECONDS, payload);
      return;
    } catch {
      // 回退到本地状态存储
    }
...
async function saveOAuthStatusSnapshot(state: string, value: OAuthStatusSnapshot): Promise<void> {
  const redis = getRedis();
  const payload = JSON.stringify(value);
  const ttlSeconds = Math.max(60, Math.ceil((value.expiresAt - Date.now()) / 1000));
  if (redis) {
    try {
...
async function getOAuthStatusSnapshot(state: string): Promise<OAuthStatusSnapshot | null> {
  const redis = getRedis();
  if (redis) {
    try {
...
function parseJwtPayload(token: string | null | undefined): Record<string, unknown> {
  if (!token) {
    return {};
  }
...
async function requestJson(url: string, init?: RequestInit): Promise<unknown> {
  ...
  if (!response.ok) {
    ...
    const message = pickString(record, ['error_description', 'error', 'message']) || `Request failed with status ${response.status}`;
    throw new AppError('OAUTH_REMOTE_REQUEST_FAILED', message, 502);
  }
  return payload;
}

async function exchangeCode(providerConfig: OAuthProviderConfig, code: string): Promise<Record<string, unknown>> {
...
async function detectGoogleEmail(tokenPayload: Record<string, unknown>): Promise<string> {
  const idTokenClaims = parseJwtPayload(pickString(tokenPayload, ['id_token']));
  const fromClaims = pickString(idTokenClaims, ['email', 'preferred_username', 'upn']);
  if (fromClaims) {
    return fromClaims;
  }
  ...
  if (!accessToken) {
    throw new AppError('GOOGLE_EMAIL_DETECTION_FAILED', 'Google OAuth succeeded but email address could not be detected', 502);
  }
  ...
  if (!fromProfile) {
    throw new AppError('GOOGLE_EMAIL_DETECTION_FAILED', 'Google OAuth succeeded but Gmail profile did not return an email address', 502);
  }
  return fromProfile;
}

async function detectOutlookEmail(tokenPayload: Record<string, unknown>): Promise<string> {
  const idTokenClaims = parseJwtPayload(pickString(tokenPayload, ['id_token']));
  const fromClaims = pickString(idTokenClaims, ['preferred_username', 'email', 'upn']);
  if (fromClaims) {
    return fromClaims;
  }
  ...
  const accessToken = pickString(tokenPayload, ['access_token']);
  if (!accessToken) {
    throw new AppError('OUTLOOK_EMAIL_DETECTION_FAILED', 'Microsoft OAuth succeeded but email address could not be detected', 502);
  }
  ...
  if (!email) {
    throw new AppError('OUTLOOK_EMAIL_DETECTION_FAILED', 'Microsoft OAuth succeeded but profile did not return an email address', 502);
  }
  return email;
}

async function getProviderConfig(provider: OAuthProvider): Promise<OAuthProviderConfig> {
...
function buildAuthorizationUrl(providerConfig: OAuthProviderConfig, state: string): string {
...
async function persistOAuthCompletion(state: string, stateRecord: OAuthStateRecord, result: OAuthCompletionResult): Promise<void> {
...
async function upsertOAuthMailbox(input: {
  ...
  if (exactEmail) {
    ...
    return { id: updated.id, email: updated.email, action: 'updated_exact_email' };
  }

  if (input.emailId) {
    try {
      const target = await emailService.getById(input.emailId, false);
      if (target.email.trim().toLowerCase() === input.email.trim().toLowerCase()) {
        ...
        return { id: updated.id, email: updated.email, action: 'updated_target_id' };
      }
    } catch {
      // 指定邮箱不存在时回退为新建
    }
...
async function verifyOAuthMailbox(emailId: number) {
  ...
  try {
    const result = await mailService.getEmails(credentials, { mailbox: 'INBOX', limit: 1 });
    ...
    const message = error instanceof Error && error.message ? error.message : 'Mailbox verification failed';
  ...
  async getAuthorizationStatus(input: {
    ...
    if (completion && completion.provider === provider) {
      if (completion.adminId !== input.adminId) {
        throw new AppError('OAUTH_STATUS_FORBIDDEN', 'OAuth status does not belong to current admin', 403);
      }
      return {
        provider,
        state,
  ...
  async completeAuthorization(input: {
    ...
    if (!stateRecord || stateRecord.provider !== provider) {
      return {
        provider,
        status: 'error',
        message: 'OAuth 状态无效或已过期，请重新发起连接',
      };
    }
    ...
    if (input.error) {
      const result: OAuthCompletionResult = {
        provider,
        status: 'error',
        message: `${provider === 'GMAIL' ? 'Google' : 'Microsoft'} OAuth 授权失败：${input.errorDescription || input.error}`,
      };
    ...
    if (!input.code) {
      const result: OAuthCompletionResult = {
        provider,
        status: 'error',
        message: 'OAuth 回调缺少 authorization code',
      };
      await persistOAuthCompletion(input.state, stateRecord, result);
      return result;
    }
    ...
    try {
      ...
      if (!refreshToken) {
        throw new AppError('OAUTH_REFRESH_TOKEN_MISSING', `${providerConfig.providerLabel} OAuth succeeded but refresh token is missing`, 502);
      }
      ...
      if (!verifyResult.ok) {
        const result: OAuthCompletionResult = {
          ...
          message: `${providerConfig.providerLabel} 已授权并写回邮箱，但首次收信验证失败：${verifyResult.message}`,
        };
        await persistOAuthCompletion(input.state, stateRecord, result);
        return result;
      }
      ...
      const result: OAuthCompletionResult = {
        ...
        email,
        action: mailbox.action,
        message: `${providerConfig.providerLabel} 授权成功，邮箱已自动${mailbox.action === 'created_new_email' ? '创建' : '更新'}并通过收信验证`,
      };
      await persistOAuthCompletion(input.state, stateRecord, result);
      return result;
    } catch (error) {
      const message = error instanceof AppError
        ? error.message
        : (error instanceof Error && error.message ? error.message : 'OAuth 处理失败');
      const result: OAuthCompletionResult = {
        provider,
        status: 'error',
        message,
