...
function normalizeProviderRedirectUri(provider: ManagedOAuthProvider, value: string | null | undefined): string | null {
...
function normalizeProviderScopes(provider: ManagedOAuthProvider, value: string | null | undefined): string | null {
  const normalized = normalizeOptionalString(value);
  if (provider !== 'OUTLOOK' || !normalized) {
    return normalized;
  }
...
function buildEnvConfig(provider: ManagedOAuthProvider): ProviderConfigRecord {
  if (provider === 'GMAIL') {
    return {
      ...
      redirectUri: normalizeProviderRedirectUri(provider, env.GOOGLE_OAUTH_REDIRECT_URI),
  ...
  return {
      ...
      redirectUri: normalizeProviderRedirectUri(provider, env.MICROSOFT_OAUTH_REDIRECT_URI),
...
function toSummary(config: ProviderConfigRecord, source: ProviderConfigSummary['source']): ProviderConfigSummary {
  return {
...
function parseGoogleClientSecretJson(input: string, callbackUri?: string | null): ParsedGoogleClientSecret {
  ...
  if (!webClient || typeof webClient !== 'object') {
    throw new AppError('GOOGLE_CLIENT_SECRET_JSON_INVALID', 'Google client secret JSON must contain a web object', 400);
  }

  const clientId = normalizeOptionalString(typeof webClient.client_id === 'string' ? webClient.client_id : null);
  const clientSecret = normalizeOptionalString(typeof webClient.client_secret === 'string' ? webClient.client_secret : null);
  const redirectUris = Array.isArray(webClient.redirect_uris)
    ? webClient.redirect_uris.filter((item): item is string => typeof item === 'string' && item.trim().length > 0).map((item) => item.trim())
    : [];

  if (!clientId || !clientSecret || redirectUris.length === 0) {
    throw new AppError('GOOGLE_CLIENT_SECRET_JSON_INVALID', 'Google client secret JSON is missing client_id, client_secret, or redirect_uris', 400);
  }
  ...
  return {
    ...
    redirectUris,
    projectId: normalizeOptionalString(typeof webClient.project_id === 'string' ? webClient.project_id : null),
  };
}
...
export const emailOAuthConfigService = {
  ...
  async getEffectiveConfig(provider: string): Promise<{ config: ProviderConfigRecord; source: ProviderConfigSummary['source'] }> {
  ...
  async saveProviderConfig(input: SaveProviderConfigInput): Promise<ProviderConfigSummary> {
    ...
    const nextClientId = input.clientId === undefined ? (current?.clientId || null) : normalizeOptionalString(input.clientId);
    const nextRedirectUri = input.redirectUri === undefined ? (current?.redirectUri || null) : normalizeProviderRedirectUri(provider, input.redirectUri);
    const nextScopes = input.scopes === undefined ? (current?.scopes || null) : normalizeProviderScopes(provider, input.scopes);
    const nextTenant = provider === 'OUTLOOK'
      ? (input.tenant === undefined ? (current?.tenant || null) : normalizeOptionalString(input.tenant))
      : null;
  ...
  async parseGoogleClientSecret(input: { filePath?: string | null; jsonText?: string | null; callbackUri?: string | null }): Promise<ParsedGoogleClientSecret> {
    ...
    if (!filePath) {
      throw new AppError('GOOGLE_CLIENT_SECRET_SOURCE_REQUIRED', 'Provide either Google client secret JSON text or a readable file path', 400);
    }
    ...
    try {
      ...
      throw new AppError('GOOGLE_CLIENT_SECRET_PATH_UNREADABLE', `Cannot read Google client secret JSON from path: ${filePath}`, 400);
    }
