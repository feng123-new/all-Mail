...
function validateImapSmtpProviderConfig(
	...
	const mergedConfig = mergeProviderConfigForProfile(
		profile,
		value.providerConfig as MailProviderConfig | null | undefined,
	);
	...
	if (
		mergedConfig.imapPort !== undefined &&
		(!Number.isInteger(mergedConfig.imapPort) || mergedConfig.imapPort <= 0)
	) {
		ctx.addIssue({
			code: z.ZodIssueCode.custom,
	...
	if (
		mergedConfig.smtpPort !== undefined &&
		(!Number.isInteger(mergedConfig.smtpPort) || mergedConfig.smtpPort <= 0)
	) {
		ctx.addIssue({
			code: z.ZodIssueCode.custom,
...
function validateProviderCredentials(
	...
	const profileSummary = getProviderProfileSummary(
		resolveProviderProfile(value.provider, authType),
	);
	...
	if (profileSummary.capabilitySummary.usesOAuth) {
		if (isCreate && !value.clientId) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				path: ["clientId"],
				message: `${value.provider} requires clientId`,
			});
		}
		if (isCreate && !value.refreshToken) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				path: ["refreshToken"],
				message: `${value.provider} requires refreshToken`,
			});
		}
	}
	...
	if (!profileSummary.capabilitySummary.usesOAuth) {
		if (isCreate && !value.password) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				path: ["password"],
				message: `${value.provider} requires password or authorization code`,
			});
		}
	}
}
...
export const updateEmailSchema = z
	.object({
		...
		refreshToken: z
			...
			.optional(),
		clientSecret: z
			.union([clientSecretSchema, z.literal(""), z.null()])
			.optional(),
		password: z.union([z.string().min(1), z.literal(""), z.null()]).optional(),
		accountLoginPassword: z
			.union([z.string().min(1), z.literal(""), z.null()])
			.optional(),
...
export const batchFetchMailboxesSchema = z.object({
	...
	representativeProtocol: representativeProtocolEnum.optional(),
	mailboxes: z
		.array(emailMailboxEnum)
		.min(1)
		.default(["INBOX", "SENT", "Junk"]),
});
...
const revealableEmailSecretFieldEnum = z.enum([
	"password",
	"refreshToken",
...
export const revealEmailSecretsSchema = z
	.object({
...
export type BatchFetchMailboxesInput = z.infer<
	typeof batchFetchMailboxesSchema
>;
export type BatchClearMailboxInput = z.infer<typeof batchClearMailboxSchema>;
export type DeleteSelectedMailsInput = z.infer<
	typeof deleteSelectedMailsSchema
>;
