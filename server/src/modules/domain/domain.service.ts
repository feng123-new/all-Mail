...
import {
	...
	getSavedCloudflareToken,
	mergeCloudflareValidationIntoDnsStatus,
	parseDomainDnsStatus,
...
import type {
	...
	SaveCloudflareValidationConfigInput,
...
export const domainService = {
	...
	async getById(id: number) {
		...
		return {
			...domain,
			dnsStatus: toSafeDomainDnsStatus(domain.dnsStatus),
			cloudflareValidation: getCloudflareValidationView(domain.dnsStatus),
	...
	async configureVerification(
		id: number,
		input: ConfigureDomainVerificationInput,
	) {
		const existing = await prisma.domain.findUnique({ where: { id } });
		if (!existing) {
			throw new AppError("DOMAIN_NOT_FOUND", "Domain not found", 404);
		...
		return prisma.domain.update({
			...
			data: {
				verificationToken:
					input.verificationToken?.trim() || randomBytes(12).toString("hex"),
			},
			select: {
				id: true,
				name: true,
	...
	async saveCloudflareValidationConfig(
		id: number,
		input: SaveCloudflareValidationConfigInput,
	) {
		const existing = await prisma.domain.findUnique({
			where: { id },
			select: { id: true, dnsStatus: true },
		});
		if (!existing) {
			throw new AppError("DOMAIN_NOT_FOUND", "Domain not found", 404);
		}

		const dnsStatus = saveCloudflareConfigToDnsStatus(
			existing.dnsStatus,
			input,
		);
		const updated = await prisma.domain.update({
			where: { id },
			data: {
				dnsStatus: dnsStatus as unknown as Prisma.InputJsonValue,
			},
			select: { id: true, dnsStatus: true, updatedAt: true },
		});

		return {
			id: updated.id,
			updatedAt: updated.updatedAt,
			dnsStatus: toSafeDomainDnsStatus(updated.dnsStatus),
			cloudflareValidation: getCloudflareValidationView(updated.dnsStatus),
		};
	},

	async validateCloudflare(id: number) {
		const domain = await prisma.domain.findUnique({
			where: { id },
			select: {
				id: true,
		...
		const apiToken = getSavedCloudflareToken(domain.dnsStatus);
		if (!apiToken) {
			throw new AppError(
				"CLOUDFLARE_TOKEN_REQUIRED",
				"Save a Cloudflare API token before running validation",
				400,
			);
		}

		const validation = await validateCloudflareDomain({
			domainName: domain.name,
			canReceive: domain.canReceive,
			isCatchAllEnabled: domain.isCatchAllEnabled,
			apiToken,
			zoneId: parseDomainDnsStatus(domain.dnsStatus).cloudflare?.zoneId || null,
		});

		const nextDnsStatus = mergeCloudflareValidationIntoDnsStatus(
			domain.dnsStatus,
			validation,
		);
		const updated = await prisma.domain.update({
			where: { id },
			data: {
				dnsStatus: nextDnsStatus as unknown as Prisma.InputJsonValue,
			},
			select: { id: true, dnsStatus: true, updatedAt: true },
		});
	...
	async configureCatchAll(id: number, input: ConfigureCatchAllInput) {
		...
		if (input.isCatchAllEnabled) {
			if (!input.catchAllTargetMailboxId) {
				throw new AppError(
					"CATCH_ALL_TARGET_REQUIRED",
					"Catch-all target mailbox is required when enabling catch-all",
					400,
				);
			}
			...
			if (!mailbox || mailbox.domainId !== id) {
				throw new AppError(
					"DOMAIN_MAILBOX_NOT_FOUND",
					"Catch-all target mailbox was not found in this domain",
					404,
				);
			}
			if (mailbox.status !== "ACTIVE") {
				throw new AppError(
					...
					"Catch-all target mailbox must be active",
					400,
				);
			}
		}
		...
		return prisma.domain.update({
			...
			data: {
				isCatchAllEnabled: input.isCatchAllEnabled,
				catchAllTargetMailboxId: input.isCatchAllEnabled
					? (input.catchAllTargetMailboxId ?? null)
					: null,
			},
			select: {
				id: true,
	...
	async saveSendingConfig(id: number, input: SaveDomainSendingConfigInput) {
		...
		if (!domain.canSend) {
			throw new AppError(
				...
				"This domain is receive-only and cannot be configured for outbound sending",
				400,
			);
		}
		...
		if (!existingConfig && !input.apiKey) {
			throw new AppError(
				"SEND_API_KEY_REQUIRED",
				"An API key is required when creating a sending configuration",
				400,
			);
		}
		...
		type DomainSendingConfigSummary = Prisma.DomainSendingConfigGetPayload<{
		...
		if (existingConfig) {
			result = await prisma.domainSendingConfig.update({
				...
				data: {
					...
					fromNameDefault:
						input.fromNameDefault === undefined
							? undefined
							: input.fromNameDefault?.trim() || null,
					replyToDefault:
						input.replyToDefault === undefined
							? undefined
							: input.replyToDefault?.trim() || null,
					status: "ACTIVE",
				},
				select,
			});
	...
	async createAlias(id: number, input: CreateMailboxAliasInput) {
		...
		if (!mailbox || mailbox.domainId !== id) {
			throw new AppError(
				"DOMAIN_MAILBOX_NOT_FOUND",
				"Mailbox was not found in this domain",
				404,
			);
		}
		...
		const existing = await prisma.mailboxAlias.findUnique({
			where: { aliasAddress },
		});
		if (existing) {
	...
	async delete(id: number) {
		...
		const [mailboxCount, inboundCount, outboundCount] = await Promise.all([
			prisma.domainMailbox.count({ where: { domainId: id } }),
			prisma.inboundMessage.count({ where: { domainId: id, isDeleted: false } }),
			prisma.outboundMessage.count({ where: { domainId: id } }),
		]);

		if (mailboxCount > 0 || inboundCount > 0 || outboundCount > 0) {
			throw new AppError(
				"DOMAIN_NOT_EMPTY",
				"Cannot delete a domain that still has mailboxes or message history",
				400,
			);
		}
