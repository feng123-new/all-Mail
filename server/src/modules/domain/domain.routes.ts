...
const domainRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    fastify.post('/:id(^\\d+$)/verify', async (request) => {
        ...
        const result = await domainService.configureVerification(Number.parseInt(id, 10), input);
        return { success: true, data: result };
    });
    ...
    fastify.post('/:id(^\\d+$)/catch-all', async (request) => {
        ...
        const result = await domainService.configureCatchAll(Number.parseInt(id, 10), input);
        return { success: true, data: result };
    });
    ...
    fastify.post('/:id(^\\d+$)/sending-config', async (request) => {
        ...
        const result = await domainService.saveSendingConfig(Number.parseInt(id, 10), input);
        return { success: true, data: result };
    });
    ...
    fastify.get('/:id(^\\d+$)/aliases', async (request) => {
        ...
        const result = await domainService.listAliases(Number.parseInt(id, 10), input);
        return { success: true, data: result };
    });
    ...
    fastify.post('/:id(^\\d+$)/aliases', async (request) => {
        ...
        const result = await domainService.createAlias(Number.parseInt(id, 10), input);
        return { success: true, data: result };
    });
    ...
    fastify.patch('/:id(^\\d+$)/aliases/:aliasId(^\\d+$)', async (request) => {
        ...
        const result = await domainService.updateAlias(Number.parseInt(id, 10), Number.parseInt(aliasId, 10), input);
        return { success: true, data: result };
    });

    fastify.delete('/:id(^\\d+$)/aliases/:aliasId(^\\d+$)', async (request) => {
        const { id, aliasId } = request.params as { id: string; aliasId: string };
        const result = await domainService.deleteAlias(Number.parseInt(id, 10), Number.parseInt(aliasId, 10));
        return { success: true, data: result };
    });
