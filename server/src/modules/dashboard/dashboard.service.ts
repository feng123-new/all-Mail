...
export const dashboardService = {
    ...
    async getStats() {
        ...
        return {
            ...
            domainMail: {
                ...
                activeDomains,
                mailboxes: totalDomainMailboxes,
                activeMailboxes: activeDomainMailboxes,
                inboundMessages: totalInboundMessages,
                outboundMessages: totalOutboundMessages,
            },
        };
    },
    ...
    async deleteLog(id: number) {
        const exists = await prisma.apiLog.findUnique({
            where: { id: BigInt(id) },
            select: { id: true },
        });
        if (!exists) {
        ...
        await prisma.apiLog.delete({ where: { id: BigInt(id) } });
        return { deleted: true };
    },

    async batchDeleteLogs(ids: number[]) {
        const uniqueIds = Array.from(new Set(ids.filter((id) => Number.isInteger(id) && id > 0))).map((id) => BigInt(id));
