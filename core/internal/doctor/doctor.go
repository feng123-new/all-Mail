...
func API(ctx context.Context, cfg config.Config) error {
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, fmt.Sprintf("http://127.0.0.1:%d/readyz", cfg.Port), nil)
	if err != nil {
		return err
	}
	...
	if response.StatusCode != http.StatusOK {
		return fmt.Errorf("API readiness returned %d", response.StatusCode)
	}
	return nil
}
