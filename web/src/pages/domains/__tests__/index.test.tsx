...
vi.mock("antd", async () => {
	...
	type FormInstance = {
		...
		resetFields: () => void;
		setFields: (
			_fields: Array<{ name: string | string[]; errors?: string[] }>,
		) => void;
		submit: () => void;
		__connect: (
			submitter: (values: Record<string, unknown>) => void,
			rerender: () => void,
		) => void;
		__disconnect: () => void;
	...
	function createFormInstance(): FormInstance {
		...
		let submitter: ((nextValues: Record<string, unknown>) => void) | null =
			null;
	...
	const FormItem = ({
		...
		React.useEffect(() => {
			if (
				...
				form.__getValue(name) !== undefined
			) {
				return;
			}
				...
				const normalized =
					valuePropName === "checked"
						? typeof value === "boolean"
							? value
							: (value as { target?: { checked?: boolean } })?.target?.checked
	...
	return {
		...
		Button,
		Breadcrumb: ({ items = [] }: { items?: Array<{ title: unknown }> }) => (
			<div>
				{items.map((item) => (
					<span key={String(item.title)}>{item.title as never}</span>
				))}
			</div>
		),
		...
		Descriptions: Object.assign(
			({ children }: { children?: unknown }) => <div>{children as never}</div>,
			{
				Item: ({
					label,
					children,
					...
					<div>
						<strong>{label as never}</strong>
						{children as never}
					</div>
				),
			},
		),
		Empty: ({ description }: { description?: unknown }) => (
...
vi.mock("../../../contracts/admin/domains", () => ({
	domainsContract: {
		...
		update: vi.fn(),
		verify: vi.fn(),
		saveCloudflareConfig: vi.fn(),
		validateCloudflare: vi.fn(),
		saveCatchAll: vi.fn(),
...
describe("DomainsPage hosted_internal admin closure", () => {
	beforeEach(() => {
		vi.clearAllMocks();
		vi.mocked(domainsContract.getList).mockImplementation(
			() => ok(listResult) as never,
		);
		vi.mocked(domainsContract.getById).mockImplementation(
			() => ok(baseDetail) as never,
		);
		vi.mocked(domainsContract.getAliases).mockImplementation(
			() => ok([]) as never,
		);
		vi.mocked(domainsContract.verify).mockImplementation(
			() => ok({ success: true }) as never,
		);
		vi.mocked(domainsContract.saveCloudflareConfig).mockImplementation(
			() => ok({ success: true }) as never,
		);
		vi.mocked(domainsContract.validateCloudflare).mockImplementation(
			() => ok({ success: true }) as never,
		);
		vi.mocked(domainsContract.saveCatchAll).mockImplementation(
			() => ok({ success: true }) as never,
		);
		vi.mocked(domainsContract.saveSendingConfig).mockImplementation(
			() => ok({ success: true }) as never,
		);
		vi.mocked(domainsContract.createAlias).mockImplementation(
			() => ok({ success: true }) as never,
		);
		vi.mocked(domainsContract.delete).mockImplementation(
			() => ok({ success: true }) as never,
		);
	});
	...
	it("loads domain detail and aliases when opening the config modal", async () => {
		...
		expect(
			await screen.findByDisplayValue("verify-token-1"),
		).toBeInTheDocument();
	...
	it("creates the first sending config and refreshes the detail state", async () => {
		...
		fireEvent.change(screen.getByLabelText("默认发件人名称"), {
			target: { value: "Ops Team" },
		});
		fireEvent.change(screen.getByLabelText("默认 Reply-To"), {
			target: { value: "reply@example.com" },
		});
		fireEvent.change(screen.getByPlaceholderText("首次创建必须填写 API Key"), {
			target: { value: "rekey1234" },
		});
	...
	it("regenerates the verification token and reloads domain detail", async () => {
		...
		expect(
			await screen.findByText("当前 Token：verify-token-2"),
		).toBeInTheDocument();
	}, 10000);
	...
	it("saves cloudflare config and validates the domain setup", async () => {
		const validatedDetail = {
			...baseDetail,
			cloudflareValidation: {
				hasSavedToken: true,
				tokenHint: "Saved token ending in 7890",
				zoneId: "zone_12345678",
				lastValidatedAt: "2026-04-12T00:00:00.000Z",
				lastValidation: {
					status: "pass",
					zoneName: "example.com",
					emailRoutingStatus: "ready",
					lastValidatedAt: "2026-04-12T00:00:00.000Z",
					checks: [
						{
							key: "zone-status",
			...
			dnsStatus: {
				provider: "CLOUDFLARE",
				expectedMxConfigured: true,
				expectedIngressConfigured: true,
				cloudflare: {
					tokenHint: "Saved token ending in 7890",
					zoneId: "zone_12345678",
					lastValidatedAt: "2026-04-12T00:00:00.000Z",
		...
		vi.mocked(domainsContract.getById)
			.mockImplementationOnce(() => ok(baseDetail) as never)
			.mockImplementationOnce(() => ok(validatedDetail) as never)
			.mockImplementationOnce(() => ok(validatedDetail) as never);
		...
		fireEvent.change(screen.getByLabelText("Cloudflare Zone ID（可选）"), {
			target: { value: "zone_12345678" },
		});
		fireEvent.change(screen.getByLabelText("Cloudflare API Token"), {
			target: { value: "cloudflare-token-1234567890" },
		});
		await clickAndFlush("校验 Cloudflare 挂载");

		await waitFor(() => {
			expect(domainsContract.saveCloudflareConfig).toHaveBeenCalledWith(1, {
				apiToken: "cloudflare-token-1234567890",
				zoneId: "zone_12345678",
				clearSavedToken: undefined,
			});
			expect(domainsContract.validateCloudflare).toHaveBeenCalledWith(1);
		});
	...
	it("creates an alias with the default active mailbox and refreshes the alias list", async () => {
		...
		fireEvent.change(screen.getByLabelText("Alias 本地部分"), {
		...
		await waitFor(() => {
			expect(
				screen.queryByRole("dialog", { name: /新增 Alias/ }),
