...
`route-ownership.json` is the canonical public route-family ownership contract. It is loaded by the Go gateway before the listener starts and drives routing, response ownership headers, and bounded Prometheus migration metrics. Fastify route-prefix changes must update this file in the same pull request. Ownership changes are committed revisions, not environment toggles.
...
Internal container variables such as `DATABASE_URL`, `REDIS_URL`, `ALL_MAIL_STATE_DIR`, `ALL_MAIL_STATIC_DIR`, `BUSINESS_API_URL`, `ALL_MAIL_ROUTE_OWNERSHIP_FILE`, and fixed secret-file paths are owned by images or Compose and are intentionally absent from the operator template.
